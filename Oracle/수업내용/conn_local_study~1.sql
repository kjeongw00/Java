...
--두 개 이상 테이블을 사용해서 조회할 때
--중복된 컬럼이 존재하면 어떤 테이블에서 컬럼을
--조회할 지 결정해야하느넫 그럴 때 주로 테이블에
...
--5. 연결연산자(||)
--5-1. 사원 이름 급여 연결해서 출력(EMP)
SELECT ENO 
    , ENAME || '-' || SAL
    FROM EMP;
    
--5-2. 학생번호랑-기말고사 성적(SCORE)
SELECT SNO || '-' || RESULT AS "학생번호 기말고사 성적"
    FROM SCORE;
--5-3. 학생번호 : 학생이름 연결해서 출력(STUDENT)
SELECT SNO || ':' || SNAME AS "학생번호 학생이름"
    FROM STUDENT;
