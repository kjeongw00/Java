...
SELECT MAX(SAL)
    ...
    FROM EMP
    NATURAL JOIN DEPT
    GROUP BY DNO, DNAME -- GROUP BY 함수 안씀!~
    HAVING MAX(SAL) >= 4000;
...
WITH
    SCORECHE AS (SELECT CNO
    , COURSE.CNAME
    , SCORE.RESULT
    , SCORE.SNO
    FROM SCORE NATURAL JOIN COURSE WHERE CNAME LIKE '%화학%'),
    STCHE AS (SELECT * FROM STUDENT WHERE MAJOR = '화학')
SELECT SNO
    ,  STCHE.SNAME
    ,  ROUND(AVG(SCORECHE.RESULT), 2)
    FROM SCORECHE
    NATURAL JOIN STCHE
    GROUP BY SNO, STCHE.SNAME;
