...
--            원래 DUAL 테이블 소유자는 SYS로 되어있는데 모든 유저에서 접근 가능
...
SELECT LPAD(ENAME, 10, '*') --사원명 앞에 *을 붙여서 총길이를 10으로 만듬
    ,  RPAD(ENAME, 10, '*') --사원명 뒤에 *을 붙여서 총길이를 10으로 만듬
  FROM EMP;
...
SELECT TRANSLATE('World of Warcraft', 'Wo', '--') --문자 단위로 동작하기 때문에 모든 W, o 치환
     , REPLACE('World of Warcraft', 'Wo', '--') --문자열 단위로 동작하기 때문에 Wo이 치환
    FROM DUAL;
...
SELECT ROUND(TO_DATE('20230424 00:00:00', 'yyyymmdd HH24:mi:ss'), 'YYYY')
    FROM DUAL;
...
SELECT TO_CHAR(100000000, '999,999,999') --9자리까지 숫자를 표기하되 3자리마다 ,를 표출
    ...
    SELECT TO_CHAR(1000000, '099,999,999') --9자리까지 숫자를 표기하되 3자리마다 ,를 표출하고 앞 자리에 0을 붙여서 출력
...
SELECT TO_NUMBER('-123.456', '999.999') --문자열 자리수보다 형식지정자는 자리수를 같거나 더 많게 지정해줘야한다.
    FROM DUAL;
...
SELECT *
    FROM EMP
    WHERE HDATE < TO_CHAR('19920201');

--송강교수의 임용일자를 xxxx년 xx월 xx일 xx요일입니다. 조회(TO_CHAR)
SELECT PNAME 
    , TO_CHAR(HIREDATE, 'YYYY"년 "MM"월 "DD"일 "DAY"입니다"') AS 임욜일자
    FROM PROFESSOR
    WHERE PNAME = '송강';
...
SELECT ENO
    ...
    ,  DECODE(JOB,
                ...
                '지원', SAL * 1.1, --업무가 개발, 경영이 아닌 사람들중 업무가 지원인 사람들
                '분석', SAL,       --업무가 개발, 경영, 지원이 아닌 사람들중 업무가 분석인 사람들 
                        SAL * 0.95--업무가 개발, 경영, 지원, 분석이 아닌 사람들
            ) AS CHANGE_SAL
    FROM EMP;
...
SELECT ENO
    ...
    ,  CASE JOB
        ...
        WHEN '지원' THEN SAL * 1.1 --업무가 개발, 경영이 아닌 사람들중 업무가 지원인 사람들
        WHEN '분석' THEN SAL       --업무가 개발, 경영, 지원이 아닌 사람들중 업무가 분석인 사람들 
        ELSE SAL * 0.95           --업무가 개발, 경영, 지원, 분석이 아닌 사람들
